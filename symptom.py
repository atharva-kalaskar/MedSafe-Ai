...
def analyze_side_effects(medicines: list, age: int, gender: str, reported_symptom: str) -> list:
    ...
    for med in medicines:
        ...
        for med_key, med_data in __import__('med_db').MED_DB.items():
            if med_key == med:
...
# symptom_advice = {
#     "headache": "Rest in a quiet, dark room, drink plenty of water, and try a cold compress. Consult a doctor if severe or sudden.",
#     "fever": "Stay hydrated, rest, and use a light blanket. Take fever-reducing medication if needed and see a doctor if it exceeds 103°F (39.4°C).",
#     "cough": "Drink warm fluids, use honey (if >1 year old), and rest. See a doctor if it lasts more than 3 weeks or brings up blood.",
#     "nausea": "Eat small, bland meals, avoid strong smells, and stay hydrated. Seek help if you can't keep liquids down for 24 hours.",
#     "chest pain": "URGENT: This can be serious. If accompanied by shortness of breath, pain in arms/jaw, or sweating, call emergency services immediately."
# }

# def analyze_symptom(symptom: str) -> str:
#     """Analyzes a symptom string and returns advice using fuzzy matching."""
#     symptom = symptom.lower()
    
#     # Try fuzzy matching using rapidfuzz
#     match = process.extractOne(symptom, symptom_advice.keys())
    
#     if match and match[1] >= 80: # 80% match threshold
#         matched_symptom = match[0]
#         return f"({matched_symptom.capitalize()}) " + symptom_advice[matched_symptom]
    
#     return "Symptom not recognized in our basic database. Please consult a healthcare professional for advice."
